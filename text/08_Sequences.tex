...
\mnote{.62}{\textbf{Factorial:} The expression $4!$ refers to the number $4\cdot3\cdot2\cdot1 = 24$.
...
\mnote{.84}{\textbf{Note:} It is sometimes useful to call a monotonically increasing sequence \emph{strictly increasing} if $a_n < a_{n+1}$ for all $n$; i.e, we remove the possibility that subsequent terms are equal.
...
\item		Again, the plot in Figure \ref{fig:seq7d}(c) shows that the sequence is not monotonic, but it suggests that it is monotonically decreasing after the first term. We perform the usual analysis to confirm this.
