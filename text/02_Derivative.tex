...
%
%If $f$ is differentiable on $(a,b)$, and the one-sided limits 
%$$\lim_{h\to0^+}\frac{f(a+h)-f(a)}{h}\quad\text{and}\quad\lim_{h\to0^-}\frac{f(b+h)-f(b)}{h}$$ exist, we say \sword{$f$ is differentiable on $[a,b]$.}
}
...
Recall from Section \ref{sec:limit_analytically} that $ \lim_{x\to 0}\frac{\sin x}x =1$, meaning for values of $x$ near 0, $\sin x \approx x$. Since the slope of the line $y=x$ is 1 at $x=0$, it should seem reasonable that ``the slope of $f(x)=\sin x$'' is near 1 at $x=0$. In fact, since we \textit{approximated} the value of the slope to be $0.9983$, we might guess the \textit{actual value} is 1. We'll come back to this later.\\
...
Finally, in later sections, having the derivative defined on closed intervals will prove  useful. One such place is Section \ref{sec:arc_length} where the derivative plays a role in measuring the length of a curve.
...
$$\lim_{h\to0^+}\frac{f(a+h)-f(a)}{h}\quad\text{and}\quad\lim_{h\to0^-}\frac{f(b+h)-f(b)}{h}$$ exist. Then we say \sword{$f$ is differentiable on $[a,b]$.}
}
