...
means that given any $M > 0$, there exists $\delta > 0$ such that for all $x$ in $I$, where $x\neq c$,  
if  $|x - c| < \delta$, then $f(x) >M$.
...
$$\displaystyle \lim_{x\rightarrow c^-} f(x) = \infty,$$
means  given any $M > 0$, there exists $\delta > 0$ such that for all \phantom{all all}
$a<x<c$,  
if  $|x - c| < \delta$, then $f(x) >M$.
...
{Let $I$ be an interval containing $c$ and let $f$ be a function defined on $I$, except possibly at $c$, or let $c$ be an endpoint of $I$.
