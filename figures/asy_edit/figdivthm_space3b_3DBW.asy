...
triple f(pair t) {
	return ((2+.1cos(10t.y))*cos(t.y)*sin(t.x),(2+.1cos(10t.y))*sin(t.y)*sin(t.x),(2+.1cos(10t.x))*cos(t.x));//
}
surface s=surface(f,(0,-pi),(pi,pi),32,16,usplinetype=new splinetype[] {notaknot,notaknot,monotonic},vsplinetype=new splinetype[] {notaknot,notaknot,monotonic});
pen p=rgb(0,0,.7);
draw(s,rgb(.6,.6,1)+opacity(.5),meshpen=invisible);
