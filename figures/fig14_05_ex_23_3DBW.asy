...
surface s=surface(f,(-1,-1),(1,2),8,8,usplinetype=new splinetype[] {notaknot,notaknot,monotonic});
pen p=rgb(0,0,.7);
draw(s,emissive(surfacepen),meshpen=apexmeshpen);

//Draw paraboloid over unit disk
triple f(pair t) {
	return (t.x,-1,t.y*(1-t.x^2));//
}
...
surface s=surface(f,(-1,-1),(1,2),2,2,Spline);
pen p=rgb(0,0,.7);
draw(s,emissive(surfacepen),meshpen=apexmeshpen);


triple g(real t) {return (t,-1,1-t^2);}
...
draw((1,-1,0)--(1,2,0)--(-1,2,0)--(-1,-1,0)--cycle,rgb(.3,.3,.3)+linewidth(2));
