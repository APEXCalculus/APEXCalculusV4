...
surface s=surface(f,(0,0.75),(pi/2,2),16,16,Spline);
pen p=rgb(0,0,.7);
...
triple g(real t) {return (1+.5*cos(t),1+.5*sin(t),0);}
path3 mypath=graph(g,0,2*pi,operator ..); draw(mypath,dashed+blue+linewidth(0.75));
label("$(x_0,y_0,0)$",(2.2,2.2,-0.2));
draw((1.9,1.9,-0.1)--(1.1,1.1,0),linewidth(0.75),Arrow3(size=3mm));

//draw the dashed circle on the surface of radius 0.5 with label
triple g(real t) {
return (1+.5*cos(t),1+.5*sin(t),2-((1+.5*cos(t))^2)/4-((1+.5*sin(t))^2)/4);
}
path3 mypath=graph(g,0,2*pi,operator ..); draw(mypath,dashed+blue+linewidth(0.75));
label("$(x_0,y_0,L)$",(2.2,2.2,2.5));
